...
线上部署版本，
记住，
只有这个才是最终多次测试固定后的版本，其他的只是测试版本，所有重点
是这个版本

这上面打的包不好使，再虚拟机上面运行不了，还是要在bim上打
"""
    ...
    def on_created(self, event):
        ...
        if not event.is_directory:
            ...
            if event.src_path.split('\\')[-2] == '收' :
               if not event.src_path.split('\\')[-1].startswith("~$"): # 过滤掉临时文件。
                    if len(event.src_path.split('\\')[-1]) != 8:
        ...
        while True:
            try:
                ...
                if len(self.filelist)>=2:
                    ...
                    # w = n.pop()
                    # if os.path.exists(w):
                    df = pd.read_excel("C:\\ProgramData\\1.xlsx")
                        # print(df)
